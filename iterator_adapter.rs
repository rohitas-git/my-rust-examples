...
/// 
/// Follow its corresponding fn below
/// 
/// 
fn main(){
    iterator_adapters::filter_negatives_iter_filter();
    iterator_adapters::print_doubles_with_map();
    iterator_adapters::enumerate_seq_index_iter();
}
...
pub mod iterator_adapters{
    ...
    pub fn filter_negatives_iter_filter(){
        let arr = [66, -8, 43, 19, 0, -31];
        for n in arr.iter() {
            print!("{} ", n);
        }
    ...
    /// This works because the enumerate function takes an iterator and returns another iterator. 
